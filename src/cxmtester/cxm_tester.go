...
func SendMessage(scid int, msg []byte, conn *net.TCPConn) {
    ...
    case signal = <- port_chan:
      ...
      case signal = <- read_channel:
          if((signal.len != expected_pkt_len) || (signal.port != expected_port)) {
            ...
            fmt.Println("Recevied message from crossconnect on (SCID, PORT):", scid, signal.port)
      ...
      case data := <- MainChannel:
        if(data.op == TESTER_OP_TEST_READER_FAILURE) {
          ...
          if(expected == 1) {
            ...
            fmt.Println("Testcase failed. Did not receive expected packet")
            val[5] = 1
          }
          test_done = 1
        }
