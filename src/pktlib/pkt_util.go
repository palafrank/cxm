...
func GetPktSignature(sigOffset int, sigLen int, msg []byte) []byte{
  //SigOffset is from the end of the packet
  //Siglen is to take it to the beginning of the signature
  sig := msg[(len(msg)-(sigOffset + sigLen)):(len(msg) - sigOffset)]
  return sig
}

func SetPktSignature(sigOffset int, sigLen int, msg []byte, sig []byte) {
  index := len(msg)-(sigOffset + sigLen)
